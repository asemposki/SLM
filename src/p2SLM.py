    ...
    def fit(self):
        ...
        for file in self.fileList:
            ...
            if not np.all(np.isfinite(data)):
                ...
                continue
            name_parts = file.strip("MR.dat").split("_")
...
def main(
    ...
    tov_data_path = os.path.join(TOV_DATA_PATH, "MSEOS" if mseos else "QEOS")
    files = [f for f in os.listdir(tov_data_path) if f.endswith(".dat")]
    train_files = sorted(files)[::4]
    test_files = sorted(
        [f for i, f in enumerate(files) if i % 5 == 0 and f not in train_files]
    )
