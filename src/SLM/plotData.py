...
def plot_parametric_old(Xdmd, X, name, tidal=False, xlim=None, ylim=None):
    """Makes plots for the dmds"""
    # newT = np.delete(t, -1)
    names = name.split("_")
    print("Names", names)
...
def plot_parametric(Xdmd, X_exact, name, tidal):
    """
    Plots the reconstructed DMD data against the exact data.
    Xdmd: Reconstructed data (variables x snapshots)
    X_exact: Exact data (variables x snapshots)
    name: Base name for the plot file
    tidal: Boolean, indicates if tidal data is being plotted (affects labels)
    """
    # Assuming Xdmd and X_exact are already transposed to (variables, snapshots)
    # and time column is removed.

    # Create time vector for plotting
    # Assuming the number of snapshots is the second dimension of the data
    num_snapshots = Xdmd.shape[1]
    # Assuming dt = 1.0, adjust if you have a different dt for plotting
    time_points = np.arange(num_snapshots) * 1.0

    fig, axs = plt.subplots(Xdmd.shape[0], 1, figsize=(10, 8), sharex=True)
    if Xdmd.shape[0] == 1:  # Handle case of single variable
        axs = [axs]

    # Labels based on whether it's tidal data or not
    # Adjust these labels based on your actual variable meanings
    if tidal:
        var_labels = ["R_b", "M_b", "k2_tidal"]
    else:
        var_labels = [
            "Variable 1",
            "Variable 2",
    ...
    for i in range(Xdmd.shape[0]):  # Loop through each variable (row in Xdmd)
        axs[i].plot(time_points, X_exact[i].real, label="Exact", color=colors[i])

        # THIS IS THE LINE TO CHANGE:
        # Before: np.exp(Xdmd[i + 2].real)
        # After:  np.exp(Xdmd[i].real)
        axs[i].plot(
            time_points,
            np.exp(Xdmd[i].real),
    ...
    filename = os.path.join(plot_dir, f"SLMpredict_{name}.png")
